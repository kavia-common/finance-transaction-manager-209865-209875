...
# Minimal PostgreSQL startup script with full paths + schema/seed application
DB_NAME="myapp"
DB_USER="appuser"
...
# Function: apply schema and seed idempotently
apply_schema_and_seed() {
    echo "Checking for existing users table to determine if schema needs applying..."
    local exists=$(
        sudo -u postgres ${PG_BIN}/psql -p ${DB_PORT} -d ${DB_NAME} -tAc "SELECT to_regclass('public.users') IS NOT NULL;"
    )
    if [ "${exists}" = "t" ]; then
        echo "Schema already applied (users table exists). Skipping schema.sql."
    else
    ...
    echo "Applying seed.sql (idempotent)..."
    if [ -f "seed.sql" ]; then
...
if sudo -u postgres ${PG_BIN}/pg_isready -p ${DB_PORT} > /dev/null 2>&1; then
    ...
    echo "Port: ${DB_PORT}"
    echo ""
    echo "Ensuring database and user exist and permissions are set..."
    # Ensure DB exists
...
EOF
    ...
    echo ""
    echo "Script stopped - server already running. Schema/seed ensured."
    exit 0
fi
...
# Apply schema and seed idempotently
apply_schema_and_seed
